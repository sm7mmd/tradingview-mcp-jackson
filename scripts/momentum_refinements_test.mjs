...
async function main() {
  ...
  for (let ri = 0; ri < rebal.length - 1; ri++) {
    ...
    const qExcess = (key) => {
      const pool = liquid.filter(r => r[key] != null); if (pool.length < 10) return null;
      const top = [...pool].sort((a, b) => b[key] - a[key]).slice(0, Math.max(3, Math.floor(pool.length / 5)));
      return (mean(top.map(r => r.fwd)) - COST_RT) - basket;   // net excess vs basket
    };
    const e6 = qExcess('mom6'), ei = qExcess('idio'), ew = qExcess('wk52');
    if (e6 != null) series.mom6.push(e6);
    if (ei != null) series.idio.push(ei);
    if (ew != null) series.wk52.push(ew);
  ...
  const order = [['mom6', '6-1mo momentum (baseline)'], ['idio', 'idiosyncratic momentum'], ['wk52', '52-week-high proximity']];
  for (const [k, label] of order) {
    ...
    console.log(`  ${label.padEnd(28)} excess/pd ${pct(mean(x)).padStart(8)}  CAGR ${pct(cagrM(x)).padStart(8)}  ${v.pass ? 'PASS' : 'fail'} (t ${v.t.toFixed(2)})  | OOS halves t ${h1.t.toFixed(2)} / ${h2.t.toFixed(2)} ${h1.t > 1.5 && h2.t > 1.5 ? '✓both>1.5' : '⚠'}`);
  }
